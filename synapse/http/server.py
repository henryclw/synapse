...
# successfully cancelled the request. Used only for logging purposes. Clients will only
# observe this code if cancellations leak across requests or we raise a `CancelledError`
# ourselves.
...
_cancellable_methods = frozenset(
    {
    ...
    Usage:
        class SomeServlet(RestServlet):
            ...
            async def on_GET(self, request: SynapseRequest) -> ...:
                ...
    """
    if func.__name__ not in _cancellable_methods:
        raise ValueError(
            "@cancellable decorator can only be applied to servlet methods."
        )

    func.cancellable = True  # type: ignore[attr-defined]
    return func
